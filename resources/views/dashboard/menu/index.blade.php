        ...
        <div class="container content-wrapper">
            ...
            <div class="content info-box">
                @if(count($menus) > 0)
                    ...
                    or
                @endif
                <a href="{{route('dashboard.menu.create')}}">Create a new menu</a>.
            </div>
                        ...
                        <div id="menu-content">
                                            ...
                                            @foreach($menuitems as $key=>$item)
                                                <li data-id="{{$item->id}}"><span class="menu-item-bar"><i class="fa fa-arrows"></i> @if(empty($item->name)) {{$item->title}} @else {{$item->name}} @endif <a href="#collapse{{$item->id}}" class="pull-right" data-toggle="collapse"><i class="caret"></i></a></span>
                                                    <div class="collapse" id="collapse{{$item->id}}">
                                                        <div class="input-box">
                                                            <form method="post" action="{{url('dashboard/menu/update-menuitem')}}/{{$item->id}}">
                                                                {{csrf_field()}}
                                                                <div class="form-group">
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <a href="{{url('dashboard/menu/delete-menuitem')}}/{{$item->id}}/{{$key}}" class="btn btn-sm btn-danger">Delete</a>
                                                                </div>
                                                            </form>
                                                        </div>
                                                    </div>
                                                                ...
                                                                @foreach($m as $in=>$data)
                                                                    <li data-id="{{$data->id}}" class="menu-item"> <span class="menu-item-bar"><i class="fa fa-arrows"></i> @if(empty($data->name)) {{$data->title}} @else {{$data->name}} @endif <a href="#collapse{{$data->id}}" class="pull-right" data-toggle="collapse"><i class="caret"></i></a></span>
                                                                        <div class="collapse" id="collapse{{$data->id}}">
                                                                            <div class="input-box">
                                                                                <form method="post" action="{{url('dashboard/menu/update-menuitem')}}/{{$data->id}}">
                                                                                    {{csrf_field()}}
                                                                                    <div class="form-group">
                                                                                        <label>Link Name</label>
                                                                                    ...
                                                                                    <div class="form-group">
                                                                                        <button class="btn btn-sm btn-primary">Save</button>
                                                                                        <a href="{{url('dashboard/menu/delete-menuitem')}}/{{$data->id}}/{{$key}}/{{$in}}" class="btn btn-sm btn-danger">Delete</a>
                                                                                    </div>
                                                                                </form>
                                                                            </div>
                                                                        </div>
                            ...
                            @if($desiredMenu != '')
                                <div class="form-group menulocation">
                                    ...
                                    <p><label><input type="radio" name="location" value="3" @if($desiredMenu->location == 3) checked @endif> Footer</label></p>
                                </div>
                                <div class="text-right">
                                    <button class="btn btn-sm btn-primary" id="saveMenu">Save Menu</button>
                                </div>
                                <p><a href="{{url('dashboard/menu/delete-menu')}}/{{$desiredMenu->id}}">Delete Menu</a></p>
                            @endif
                        </div>
                    @endif
                </div>
            ...
            <script>
                $('#add-categories').click(function(){
                    ...
                    $.ajax({
                        ...
                        url: "{{route('dashboard.menu.add-categories')}}",
                ...
                $('#add-posts').click(function(){
                    ...
                    $.ajax({
                        type:"get",
                        data: {menuid:menuid,ids:ids},
                        url: "{{route('dashboard.menu.add-post')}}",
                        success:function(res){
                            location.reload();
                        }
                    })
                ...
                $("#add-custom-link").click(function(){
                    ...
                    if(url.length > 0 && link.length > 0){
                        $.ajax({
                            ...
                            url: "{{route('dashboard.menu.add-link')}}",
                            success:function(res){
                                location.reload();
                            }
                        })
            ...
            <script>
                $('#saveMenu').click(function(){
                    ...
                    $.ajax({
                        ...
                        url: "{{url('dashboard/menu/update-menu')}}",
                        success:function(res){
